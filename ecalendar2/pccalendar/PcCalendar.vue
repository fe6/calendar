<template>
  <div class="w-pc-calendar">
    <div class="w-pc-calendar-header">
      <div class="w-pc-calendar-btn w-pc-calendar-prev" :class="{
        'w-pc-calendar-btn-disabled': prevDisabledValue,
        }" @click="pcPrev" @touchend="wapPrev($event)">
        <i class="w-pc-calendar-arrow w-pc-calendar-left" :class="{
          'w-pc-calendar-arrow-disabled': prevDisabledValue,
          }"></i>
      </div>
      <div class="w-pc-calendar-btn w-pc-calendar-next" :class="{
        'w-pc-calendar-btn-disabled': nextDisabledValue,
        }" @click="pcNext" @touchend="wapNext($event)">
        <i class="w-pc-calendar-arrow w-pc-calendar-right" :class="{
    ...
    <div class="w-pc-calendar-panel">
      <div class="w-pc-calendar-date w-pc-calendar-prev" v-for="day in lastMonthDays">
        <slot name="prevday" v-bind:day="day" v-if="hasScopedSlotPrevDay">
          {{ day }}
        </slot>
        <span class="w-pc-calendar-num" v-else>{{ day.num }}</span>
      </div>
      <div class="w-pc-calendar-date" :class="{
        'w-pc-calendar-disabled': day.undo,
        'w-pc-calendar-today': day.today,
        'w-pc-calendar-choice': day.choice,
        ...
        <!-- 默认 -->
        <div class="w-pc-calendar-num" v-if="!hasScopedSlotDay" ref="dayItem">
          <span class="w-pc-calendar-inner" :class="{
            'w-pc-calendar-active': day.active,
            }">{{ day.num }}</span>
        </div>
        <p class="w-pc-calendar-text" v-if="day.choice && !day.active && !hasScopedSlotDay">{{ choiceText }}</p>
      </div>
      <div class="w-pc-calendar-date w-pc-calendar-next" v-for="day in nextMonthDays">
